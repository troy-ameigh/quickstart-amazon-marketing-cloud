...
class WorkflowInterface():
    ...
    def set_workflow_schedule(self, workflowId: str, customerId: str, scheduleName: str, scheduleDetails: dict):
        '''
        ...
        if 'payload' in scheduleDetails:
            if len(scheduleDetails['payload']) > 0:
    ...
    def set_workflow_library_record(
        ...
        schedule: {
            ...
            scheduleDetails: {
                ...
                state: 'ENABLED' || 'DISABLED'
                scheduleExpression: str eg. 'custom(D * 11)'
            }
        }
        '''
        ...
        if schedule:
            if 'scheduleName' not in schedule:
               ...
               schedule['Name'] = schedule.pop('scheduleName')

            if 'scheduleDetails'['scheduleExpression'] not in schedule:
                print('Error: a scheduleExpression must be passed in to scheduleDetails')
                return

            if 'scheduleDetails'['state'] not in schedule or schedule['scheduleDetails']['state'].upper() not in ['ENABLED', 'DISABLED']:
                print('Error: a state (ENABLED or DISABLED) must be passed in to scheduleDetails')
                return

            #if all the parameters checks pass, format the schedule properly
            item['schedule'] = {}
            item['schedule']['Input']['paylod']['workflowId'] = workflowId
            item['schedule']['Name'] = schedule['scheduleName']
            item['schedule']['State'] = schedule['scheduleDetails']['state']
            item['schedule']['ScheduleExpression'] = schedule['scheduleDetails']['scheduleExpression']
